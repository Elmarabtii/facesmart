...
class EmployeeManagementSystem:
    ...
    def edit_employee(self):
        ...
        if selected_items:
            ...
            if employees_data:
                employee_ids = list(employees_data.keys())  # Get all employee IDs from the database
                print("Employee IDs:", employee_ids)  # Debugging statement
                if selected_employee_id in employee_ids:
                    selected_employee_data = employees_data.get(selected_employee_id)
                    print("Selected Employee Data:", selected_employee_data)  # Debugging statement
                    if selected_employee_data:
                        edit_window = EditEmployeeWindow(self.root, self, selected_employee_id, selected_employee_data)
                    else:
                        messagebox.showinfo("Error", "Employee data not found.")
                else:
                    messagebox.showinfo("Error",
                                        f"Selected employee ID '{selected_employee_id}' not found in database.")
            else:
